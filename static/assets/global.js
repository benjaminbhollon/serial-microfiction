...
const daysToAdd = ((releasedOn.find((day) => day > now.getUTCDay()) === undefined
  ? releasedOn[0]
  : releasedOn.find(
    (day) => day > now.getUTCDay(),
  )
) - now.getUTCDay() + 7) % 7;
...
window.addEventListener('scroll', () => {
  if (window.scrollY > 250) { document.getElementById('scrollToTop').style.display = 'initial'; } else { document.getElementById('scrollToTop').style.display = 'none'; }
});
